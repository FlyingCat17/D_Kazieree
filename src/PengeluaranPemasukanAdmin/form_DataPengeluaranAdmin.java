 ...
 * @author LenataHoma
 */
public class form_DataPengeluaranAdmin extends javax.swing.JInternalFrame {
     ...
     */
    public form_DataPengeluaranAdmin() {
        initComponents();
...
//        jDateChooser1.setDate(cal.getTime());
    ...
    public void loadTablePengeluaran() {
        DefaultTableModel mdl = new DefaultTableModel();
        mdl.addColumn("ID Pengeluaran");
...
//        }
//    }
    public void loadPengeluaranHariIni(){
        try {
            String sql = "SELECT SUM(tb_pengeluaran.jumlah_pengeluaran) AS Jumlah FROM tb_pengeluaran\n"
                    + "WHERE tb_pengeluaran.tgl_pengeluaran = '"+ad+"'\n"
                    + "GROUP BY tb_pengeluaran.tgl_pengeluaran";
            java.sql.Connection con = (Connection)konekdb.GetConnection();
            java.sql.Statement st = con.createStatement();
    ...
    public void loadPengeluaranBulanIni(){
        try {
            String sql = "SELECT SUM(tb_pengeluaran.jumlah_pengeluaran) AS JUMLAH FROM tb_pengeluaran\n"
                    + "WHERE EXTRACT(YEAR_MONTH FROM tb_pengeluaran.tgl_pengeluaran) = '"+ads+"'\n"
    ...
    private void initComponents() {
        ...
        jLabel9.setText("Pengeluaran Bulan Ini");
    ...
    public String setIDPemasukan(String id_pemasukan){
        this.IDPengeluaran2 = id_pemasukan;
        return id_pemasukan;
    }
    public String getIDPemasukan(){
        return this.IDPengeluaran2;
    }
...
//        System.out.println("Harga Jual "+ hargajual);
    ...
    private void button4ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_button4ActionPerformed
        ...
        if (getIDPemasukan()==null) {
            JOptionPane.showMessageDialog(this, "Harap Pilih Salah Satu Pengeluaran");
        } else {
            try {
                int jawab = JOptionPane.showConfirmDialog(this, "Yakin ingin menghapus pengeluaran ini??");
                switch(jawab){
                    case JOptionPane.YES_OPTION:
                        String sql = "DELETE FROM tb_pengeluaran WHERE id_pengeluaran = '" + getIDPemasukan() + "';";
                        java.sql.Connection con = (Connection) konekdb.GetConnection();
                        java.sql.Statement st = con.createStatement();
                        st.execute(sql);
